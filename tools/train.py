...
from pysot.utils.model_load import load_pretrain, restore_from,modified_load_pretrained
from pysot.utils.average_meter import AverageMeter
from pysot.utils.misc import describe, commit
from pysot.models.model_builder import ModelBuilder,ModelBuilderWithCBM,ModelBuilderWithCBAM
...
def build_opt_lr(model, current_epoch=0):
    ...
    trainable_params +=[
        {'params':filter(lambda x: x.requires_grad,
                                           model.se1.parameters()),
         'lr':cfg.TRAIN.BASE_LR}
    ]
    trainable_params += [
        {'params': filter(lambda x: x.requires_grad,
                                           model.se2.parameters()),
         'lr': cfg.TRAIN.BASE_LR}
    ]
    trainable_params += [
        {'params': filter(lambda x: x.requires_grad,
                                           model.se3.parameters()),
         'lr': cfg.TRAIN.BASE_LR}
    ]
    ...
    def weights_grads(model):
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                grad[name] = param.grad #save grad...
                weights[name] = param.data #save network parameters as tensor
        return grad, weights
        ...
        if rank == 0:
            ...
            if (idx+1) % cfg.TRAIN.PRINT_FREQ == 0:
                for name, para in model.named_parameters():
                    if name == 'module.se1.fc.0.weight':
                        print("se1 fc para: ", para)
                info = "Epoch: [{}][{}/{}] lr: {:.6f}\n".format(
...
def main():
    ...
    # create model
    #model = ModelBuilderWithCBM().cuda().train()
    model=ModelBuilderWithCBAM().cuda().train()
    ...
    # load pretrain
        ...
        model=modified_load_pretrained(model, cfg.TRAIN.PRETRAINED)
