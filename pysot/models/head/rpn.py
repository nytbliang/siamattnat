...
class MultiRPN(RPN):
    def __init__(self, anchor_num, in_channels, weighted=False):
        ...
        for i in range(len(in_channels)):
            self.add_module('rpn'+str(i+2),
                    DepthwiseRPN(anchor_num, in_channels[i], in_channels[i])) #add sub-module
        if self.weighted:
            self.cls_weight = nn.Parameter(torch.ones(len(in_channels))) #initialize with one ....
    ...
    def forward(self, z_fs, x_fs):
        ...
        for idx, (z_f, x_f) in enumerate(zip(z_fs, x_fs), start=2):
            rpn = getattr(self, 'rpn'+str(idx)) #is equivalent to self.rpn3 self.rpn4 self.rpn5
            c, l = rpn(z_f, x_f)
            cls.append(c)
            loc.append(l)
...
class Modefied_MultiRPN(RPN):
    def __init__(self, anchor_num, in_channels, weighted=False):
        super(Modefied_MultiRPN, self).__init__()
        self.weighted = weighted
        for i in range(len(in_channels)):
            self.add_module('rpn'+str(i+2),
                    DepthwiseRPN(anchor_num, in_channels[i], in_channels[i])) #add sub-module
    ...
    def forward(self, z_fs, x_fs):
        ...
        if self.weighted:
            ...
            return avg(cls), avg(loc),cls,loc